...
import { useUser } from "../context/UserContext"; // Only import the useUser hook
    ...
    >
      {thumbnailImg ? (
        <div className="relative w-full h-full overflow-hidden">
          ...
          {isSelected && (
            <div className="absolute inset-0 bg-black-/500/10 flex items-center justify-center">
        ...
        <div className="w-full h-[200px] flex items-center justify-center bg-gradient-to-br from-violet-50 via-violet-600 to-fuchsia-50">
          <div className="w-12 h-12 bg-gradient-to-br from-violet-500 to-fuchsia-500 rounded-2xl flex items-center justify-center mb-3"></div>
          <span className="font-bold text-gray-700">No Preview</span>
        </div>
      )}
      <div className="p-4">
        <h3 className="text-lg font-bold text-left">{name || "Template"}</h3>
...
// If you have a default export, make sure it also uses the hook
const Cards = () => {
  const { user } = useUser(); // Use the useUser hook
